...
class ImageToolsTest {
    ...
    @ParameterizedTest
    @ValueSource(strings = {"/root/file.png", "photo.jpg", "./pic.webp",})
    void isImage(String path) {
        assertTrue(ImageTools.isImage(Path.of(path)));
    }

    @ParameterizedTest
    @ValueSource(strings = {"/root/file.txt", "photo", "/.jpfeg", "./", "", "1223/file.heic", "/.jpeg", "/.jPEg",})
    void isNotImage(String path) {
        assertFalse(ImageTools.isImage(Path.of(path)));
    }

    @ParameterizedTest
    @CsvSource({"/root/file.txt, file, txt", // normal file
    ...
    void testGetFilePartsMalformed(String input) {
        ...
        Exception ex = assertThrows(
                Exception.class,
                () -> ImageTools.getFileParts(Path.of(input))
        );
